...
#     # Extract the line containing the complexity information
#     complexity_line = next((line for line in result.splitlines() if "Complexity level:" in line), None)
#     if complexity_line:
#         # Split the line at the colon and return the complexity level (High, Medium, Low)
#         return complexity_line.split(":")[1].strip()  # Extract the part after '## Complexity:'
#     return None
...
# def extract_tags_from_output(result):
#     # Find the Tags section in the result
#     tag_section = re.search(r"Tags: (.*)", result)
#     if tag_section:
#         tags = tag_section.group(1).split(",")  
...
def main():
    ...
    # render_navigation_buttons(val, 3)
        ...
        # with col1:
        #     if val > 0:
        #         st.button("Previous", on_click=lambda: set_step(val - 1))
        ...
        # with col3:
        #     if val < 3:
        #         st.button("Next", on_click=lambda: set_step(val + 1))
